...
cv::Mat ImageGrabber::next(const int &speed)
{
    std::string file = imageManager->next(speed);
    frameNumber = imageManager->getCount();
    return cv::imread(file);
}

cv::Mat ImageGrabber::prev(const int &speed)
{
    std::string file = imageManager->prev(speed);
    frameNumber = imageManager->getCount();
    return cv::imread(file);
}
